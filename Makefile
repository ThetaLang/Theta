...
SRC_FILES = $(wildcard $(SRC_DIR)/*.cpp $(LEXER_DIR)/*.cpp $(PARSER_DIR)/*.cpp $(UTIL_DIR)/*.cpp)
...
$(BUILD_LEXER_DIR) $(BUILD_PARSER_DIR) $(BUILD_UTIL_DIR) $(BUILD_AST_DIR):
	mkdir -p $@
...
$(BUILD_DIR)/%.o: %.cpp $(AST_FILES) | $(BUILD_LEXER_DIR) $(BUILD_PARSER_DIR) $(BUILD_UTIL_DIR) $(BUILD_AST_DIR)
	$(CXX) $(CXXFLAGS) -c -o $@ $<
