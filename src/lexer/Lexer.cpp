...
/**
 * @class Lexer
 * @brief A lexer for the Theta programming language that tokenizes source code into a deque of
 * Token objects.
 */
namespace Theta {
    class Lexer {
    public:
        ...
        void lex(string source) {
            ...
            while (i < source.length()) {
                ...
                // We want the line and column numbers for the beginning of the token, not the end.
                // If we were to use currentLine and currentColumn, it would give us the values for
                // the end of the token, since some of the makeToken function calls actually update
                // currentLine and currentColumn while they iterate over the token contents
                ...
                // Some tokens are more than one character. We want to advance the iterator and
                // currentLine by however many characters long the token was. We really only want to
                // do this for any token that was not created by an accumulateUntil function, since
                // those already update the index internally.
    ...
    private:
        ...
        array<Token::Types, 4> NON_EMITTED_TOKENS = {
            Token::Types::NEWLINE, Token::Types::WHITESPACE, Token::Types::COMMENT, Token::Types::MULTILINE_COMMENT};

        array<Token::Types, 7> ACCUMULATED_TOKENS = {Token::Types::IDENTIFIER,
                                                     Token::Types::KEYWORD,
                                                     Token::Types::BOOLEAN,
                                                     Token::Types::COMMENT,
        ...
        array<string, 7> LANGUAGE_RESERVED_WORDS = {Lexemes::LINK,
                                                    Lexemes::CAPSULE,
                                                    Lexemes::IF,
                                                    Lexemes::ELSE,
                                                    Lexemes::STRUCT,
        ...
        /**
         * @brief Creates a Token object based on the current and next characters in the source
         * code.
        ...
        Token makeToken(char currentChar, char nextChar, string source, int &i) {
            ...
            if (attemptLex(Lexemes::STRING_DELIMITER,
                           Token::Types::STRING,
                           token,
                           currentChar,
                           nextChar,
                           source,
                           i,
                           Lexemes::STRING_DELIMITER) ||
                attemptLex(Lexemes::COMMENT,
                           Token::Types::COMMENT,
                           token,
                           currentChar,
                           nextChar,
                           ...
                           false,
                           false) ||
                attemptLex(Lexemes::MULTILINE_COMMENT_DELIMITER_START,
                           Token::Types::MULTILINE_COMMENT,
                           token,
                           currentChar,
                           nextChar,
                           source,
                           i,
                           Lexemes::MULTILINE_COMMENT_DELIMITER_END) ||
                ...
                attemptLex(Lexemes::FUNC_DECLARATION,
                           Token::Types::FUNC_DECLARATION,
                           token,
                           currentChar,
                           nextChar,
                           source,
                ...
                attemptLex(
                    Lexemes::BRACKET_OPEN, Token::Types::BRACKET_OPEN, token, currentChar, nextChar, source, i) ||
                attemptLex(
                    Lexemes::BRACKET_CLOSE, Token::Types::BRACKET_CLOSE, token, currentChar, nextChar, source, i) ||
            ...
            if (currentChar == '\n') {
                ...
                // We default this to an identifier, but then change it later if we discover its
                // actually a keyword or bool
        ...
        /**
         * @brief Attempts to lex a token from the source code based on the given symbol and token
         * type.
         ...
         */
        bool attemptLex(const string &symbol,
            ...
            if (currentChar == symbol[0] && (symbol.length() == 1 || nextChar == symbol[1])) {
                if (terminal != "") {
                    ...
                    // in the token text field, because the combination of an accumulator and a
                    // symbol that is different from the prefix we start at causes some weird
                    // insertion to happen. We could add extra logic to fix it, but we throw out
                    // comments anyway right after they are lexed, so its probably not worth the
                    // extra code to do so. Our line numbers and columns are still correct and the
                    // lexing otherwise succeeds. However, if we ever come across a token that is
                    // *not* a comment in the future that follows the same weird capture logic,
                    // we'll need to fix it.
                    token = accumulateUntilNext(
                        terminal, source, i, Token(tokenType, symbol), appendSymbol ? symbol : "", incrementAfter);
                } else {
        ...
        /**
         * @brief Accumulates characters from the source until all of the specified end characters
         * are encountered as a substring.
         ...
         * @param append Additional text to append to the accumulated token text (default is an
         * empty string).
         * @param incrementAfter Whether to increment the index after accumulation (default is
         * true).
         * @return The updated Token object after accumulation.
         */
        Token accumulateUntilNext(
            string endChars, string source, int &i, Token token, string append = "", bool incrementAfter = true) {
            return accumulateUntilCondition(
        ...
        /**
         * @brief Accumulates characters from the source until any character from the specified
         * endChars string is encountered.
         * @param endChars A string containing characters that mark the end of accumulation.
         * @param source The source code string to lex.
         * @param i The current index in the source string.
         * @param token The Token object to update with accumulated characters.
         ...
         * @return The updated Token object after accumulation.
         */
        Token accumulateUntilAnyOf(
            string endChars, string source, int &i, Token token, string append = "", bool incrementAfter = true) {
            return accumulateUntilCondition(
                [endChars, source](int i) {
                    return find(endChars.begin(), endChars.end(), source[i]) == endChars.end();
                },
                source,
                i,
                token,
        ...
        /**
         * @brief Generalized accumulation function that continues accumulating characters as long
         * as the provided condition function returns true.
         * @param shouldContinue A function that takes an integer index and returns true if
         * accumulation should continue.
         * @param source The source code string to lex.
         * @param i The current index in the source string.
         * @param token The Token object to update with accumulated characters.
         * @param append Additional text to append to the accumulated token text (default is an
         ...
         * @return The updated Token object after accumulation.
         */
        Token accumulateUntilCondition(function<bool(int)> shouldContinue,
                                       string source,
                                       int &i,
                                       Token token,
                                       string append = "",
            ...
            for (; i < source.length() && shouldContinue(i); i++) {
                ...
                // We might hit newlines in multiline comments and strings. We need to keep line and
            ...
            // If incrementAfter is false, that means we want to roll back the index to the point
            // right before we hit an endChar. This is probably because the token we're parsing
            // isn't a token thats enclosed in delimiters, so we want to go back to processing the
            // endChar we just stopped on, because it's going to be part of another token.
            if (!incrementAfter) {
                i--;
                currentColumn--;
            }
        ...
        bool isLanguageKeyword(string lexeme) {
            return find(LANGUAGE_RESERVED_WORDS.begin(), LANGUAGE_RESERVED_WORDS.end(), lexeme) !=
                   LANGUAGE_RESERVED_WORDS.end();
        }
