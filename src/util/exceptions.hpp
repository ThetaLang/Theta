...
namespace Theta {
    ...
    public:
        CompilationError(string type, string msg, Token tok, string &src, string file)
            : errorType(type), message(msg), token(tok), source(src), fileName(file){};

        string what() {
            return message + " at line " + to_string(token.getStartLocation()[0]) + ", column " +
                   to_string(token.getStartLocation()[1]);
        }
        ...
        void display() {
            ...
            // If the error isn't on the first line of the file, we want to add a prevLine to the
            ...
            // If there's another line after this one, add its contents to the nextLine for logging
            // output
