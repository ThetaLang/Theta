        ...
        namespace Detail {
            SampleAnalysis analyse(const IConfig &cfg, FDuration *first, FDuration *last) {
                if (!cfg.benchmarkNoAnalysis()) {
                    ...
                    auto outliers =
                        Catch::Benchmark::Detail::classify_outliers(samples.data(), samples.data() + samples.size());
    ...
    namespace Benchmark {
        namespace Detail {
            namespace {
                ...
                static sample resample(
                    URng &rng, unsigned int resamples, double const *first, double const *last, Estimator &estimator) {
                ...
                static sample
                jackknife(double (*estimator)(double const *, double const *), double *first, double *last) {
...
namespace Catch {
    ...
    TestCaseInfo::TestCaseInfo(StringRef _className, NameAndTags const &_nameAndTags, SourceLineInfo const &_lineInfo)
        ...
        for (size_t idx = 0; idx < originalTags.size(); ++idx) {
            ...
            if (c == '[') {
                CATCH_ENFORCE(!inTag,
                              "Found '[' inside a tag while registering test case '" << _nameAndTags.name << "' at "
                                                                                     << _lineInfo);
            ...
            if (c == ']') {
                CATCH_ENFORCE(inTag,
                              "Found unmatched ']' while registering test case '" << _nameAndTags.name << "' at "
                                                                                  << _lineInfo);
                ...
                CATCH_ENFORCE(!tagStr.empty(),
                              "Found an empty tag while registering test case '" << _nameAndTags.name << "' at "
                                                                                 << _lineInfo);
...
namespace Catch {
    namespace Clara {
        ...
        void Parser::writeToStream(std::ostream &os) const {
            ...
            for (auto &cols : rows) {
                auto row =
                    TextFlow::Column(CATCH_MOVE(cols.left)).width(optWidth).indent(2) + TextFlow::Spacer(4) +
        ...
        Detail::InternalParseResult Parser::parse(std::string const &exeName, Detail::TokenStream tokens) const {
            ...
            auto result =
                Detail::InternalParseResult::ok(Detail::ParseState(ParseResultType::NoMatch, CATCH_MOVE(tokens)));
...
namespace Catch {
    ...
    Optional<ReporterSpec> parseReporterSpec(StringRef reporterSpec) {
        ...
        return ReporterSpec{
            CATCH_MOVE(parts[0]), CATCH_MOVE(outputFileName), CATCH_MOVE(colourMode), CATCH_MOVE(kvPairs)};
    }
...
namespace Catch {
            ...
            struct GeneratorTracker final : TestCaseTracking::TrackerBase, IGeneratorTracker {
                ...
                void close() override {
                    ...
                    const bool should_wait_for_child = [&]() {
                        ...
                        if (std::find_if(
                                m_children.begin(), m_children.end(), [](TestCaseTracking::ITrackerPtr const &tracker) {
    ...
    IResultCapture &getResultCapture() {
        ...
        else CATCH_INTERNAL_ERROR("No result capture instance");
    }
        ...
        static void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const &tests) {
            ...
            for (auto const &test : tests) {
                ...
                CATCH_ENFORCE(prev.second,
                              "error: test case \"" << infoPtr->name << "\", with tags \"" << infoPtr->tagsAsString()
                                                    << "\" already defined.\n"
