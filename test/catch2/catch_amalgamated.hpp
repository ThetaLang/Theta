...
namespace Catch {
    ...
    struct ResultWas {
        enum OfType {
            ...
            Info = 1,
            Warning = 2,
            // TODO: Should explicit skip be considered "not OK" (cf. isOk)? I.e., should it have
            // the failure bit?
            ExplicitSkip = 4,
...
namespace Catch {
    ...
    public:
        ...
        virtual bool
        sectionStarted(StringRef sectionName, SourceLineInfo const &sectionLineInfo, Counts &assertions) = 0;
...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            ...
            /// characteristics so that we can account for it in the measurement. Implementations of
            /// std::function with optimizations that aren't always applicable, like small buffer
            /// optimizations, are not uncommon. This is effectively an implementation of
            /// std::function without any such optimizations; it may be slow, but it is consistently
            /// slow.
...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            ...
            bootstrap_analysis
            analyse_samples(double confidence_level, unsigned int n_resamples, double *first, double *last);
        } // namespace Detail
    }     // namespace Benchmark
} // namespace Catch
    ...
    namespace Benchmark {
        namespace Detail {
            ...
            EnvironmentEstimate estimate_clock_resolution(int iterations) {
                auto r =
                    run_for_at_least<Clock>(clock_resolution_estimation_time, iterations, &resolution<Clock>).result;
                return {
    ...
    namespace Clara {
        ...
        namespace Detail {
            ...
            protected:
                ...
                ResultValueBase(ResultType, T const &value) : ResultBase(ResultType::Ok) { new (&m_value) T(value); }
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_EQUALITY_OPERATOR(id, op)                                                     \
    ...
    constexpr friend auto operator op(ExprLhs &&lhs, RhsT &&rhs) -> std::enable_if_t<                                  \
...
#undef Digits
        ...
        fillBitsFrom(Generator &gen) {
            ...
            static_assert(Generator::min() == 0 && Generator::max() == static_cast<gresult_type>(-1),
                          "Generator must be able to output all numbers in its result type "
                          "(effectively it must be a "
        ...
        std::enable_if_t<std::is_signed<OriginalType>::value, UnsignedType> transposeToNaturalOrder(UnsignedType in) {
        ...
        template <typename OriginalType, typename UnsignedType>
        std::enable_if_t<std::is_unsigned<OriginalType>::value, UnsignedType> transposeToNaturalOrder(UnsignedType in) {
            static_assert(sizeof(OriginalType) == sizeof(UnsignedType),
                          "reordering requires the same sized types on both sides");
            static_assert(std::is_unsigned<UnsignedType>::value, "Input type must be unsigned");
...
namespace Catch {
    ...
    class uniform_integer_distribution {
        ...
        // How many different values are there in [a, b]. a == b => 1, can be 0 for distribution
        // over all values in the type.
...
namespace Catch {
    namespace Detail {
        ...
        template <typename ForwardIter1,
                  ...
                  typename Sentinel2,
                  typename Comparator>
        bool check_element_counts(
            ForwardIter1 first_1, const Sentinel1 end_1, ForwardIter2 first_2, const Sentinel2 end_2, Comparator cmp) {
            auto cursor = first_1;
            while (cursor != end_1) {
        ...
        bool is_permutation(
            ForwardIter1 first_1, const Sentinel1 end_1, ForwardIter2 first_2, const Sentinel2 end_2, Comparator cmp) {
            // TODO: no optimization for stronger iterators, because we would also have to constrain
            // on sentinel vs not sentinel types
...
namespace Catch {
    namespace Matchers {
        ...
        namespace Detail {
            ...
            template <typename T>
            using is_matcher =
                std::is_base_of<Catch::Matchers::MatcherUntypedBase, std::remove_cv_t<std::remove_reference_t<T>>>;
            ...
            public:
                ...
                operator&&(MatchAllOfGeneric<MatcherTs...> &&lhs, MatchAllOfGeneric<MatchersRHS...> &&rhs) {
            ...
            public:
                ...
                operator||(MatchAnyOfGeneric<MatcherTs...> &&lhs, MatchAnyOfGeneric<MatchersRHS...> &&rhs) {
...
namespace Catch {
    namespace Matchers {
        ...
        template <typename Matcher>
        std::enable_if_t<Detail::is_matcher<Matcher>::value, ContainsMatcherMatcher<Matcher>>
        ...
        public:
            ...
            bool match(RangeLike &&rng) const {
                ...
                return Catch::Detail::is_permutation(
                    begin(m_desired), end(m_desired), begin(rng), end(rng), m_predicate);
            }
...
#define CATCH_REGISTER_LISTENER(listenerType)                                                                          \
    ...
    namespace {                                                                                                        \
        Catch::ListenerRegistrar<listenerType>                                                                         \
            INTERNAL_CATCH_UNIQUE_NAME(catch_internal_RegistrarFor)(#listenerType##_catch_sr);                         \
    }                                                                                                                  \
    CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
